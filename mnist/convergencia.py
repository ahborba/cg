...
if __name__ == "__main__":
    filepath = 'D:\Área de Trabalho\phyton\\train_fourier.csv'
#
 #   with open(filepath) as fp:
  #      line = fp.readline()
   #     lines = line.split(',')
    ...
    #        print(lines.__len__())
    #        a = lines[len(lines)-1]
    #        y = np.append(y, lines[len(lines)-1])
    #        lines.pop(len(lines)-1)
    #        x = np.append(x, [lines], axis=0)
    #        cnt += 1
    ...
    #print(x.size)
    #print(y.size)
    x = np.array([[1, 1, 2 ,3], [2, 2, 3 ,4], [3, 3, 4, 13], [1, 1, 3, 8], [1,1,2, 1]])
    y = np.array([[1,2], [2,3], [3,3], [1,3] , [2,2]])
    model = NN()
    model.add_layer(60)
    model.add_layer(200)
